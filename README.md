Status:  Things described here are work in progress and unstable.
...
module with connectors for eight (eg. AHT10) temperature and humidity sensors.
...
Also, all suffered from the two main problems using `pySX127x` with a new Python and OS. 
...
The `pySX127x` package and many others used the `RPi.GPIO` module which no longer works. 
Raspberry's preferred new `gpiozero` library requires lots of re-coding and is not (yet) widely used.
Further discussion of this is at forums.raspberrypi.com/viewtopic.php?p=2160578#p2160578 and elsewhere.
The simple solution used below is the `lrpi-lgpio` library (Dave Jones (waveform80) python bindings to
joan2987's C library).  
This looks like RPi.GPIO from the application side but uses the new interface on the OS/hardware side.
...
Tested working with 2025-12-04-raspios-trixie-armhf-lite.img (Raspbian 1:6.12.47-1+rpt1).
...
With ssh running remote login can be used to do the remainder.

Posibbly change the base station name:
...
The defaults in the code will not be correctIn geographic locations where 
...
```
scp pi@somewhere:SensorRecord   SensorRecord
```   
